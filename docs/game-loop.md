...
- `company.schedule.workHours` — shift length (default 8 h / 9 AM–5 PM, configurable 8 / 10 / 12 / 14 after Schedule research)
...
- **Hiring candidates** are fully replaced each day. The pool size is dynamic: **3** by default, **4** after `hr_leads_1` is researched, **5** after `hr_leads_2` (see `getCandidatePoolSize` in `src/data/hiringResearch.js`). Candidates are filtered to `unlockedResearch`
