...
export class DeskEntity extends Entity {
  ...
  _draw() {
    ...
    this._desk
      .clear()
      .roundRect(0, 16, DESK_W, DESK_H - 16, 8)
      .fill({ color: deskColor })
      .stroke({ color: deskBorder, width: 3 });

    if (this._occupied) {
      this._monitor
        .clear()
        .roundRect(DESK_W / 2 - 28, -4, 56, 40, 6)
        .fill({ color: MONITOR_COLOR })
        .stroke({ color: MONITOR_BORDER, width: 3 });
    } else {
      this._monitor.clear();
    }
  ...
  _drawScreenGlow(active) {
    ...
    if (active && this._occupied) {
      this._screenGlow
        .roundRect(DESK_W / 2 - 24, 0, 48, 32, 4)
